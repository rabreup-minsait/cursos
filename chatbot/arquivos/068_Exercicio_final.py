...
def registrar(evento, user_id, detalhe):
    if config.LOG_ATIVO:
        logging.info("%s | usuario=%s | %s", evento, user_id, detalhe)
...
def chat(pergunta: Pergunta):
    try:
        user_id = pergunta.user_id or "default"
        texto_original = pergunta.texto.strip()
        texto_normalizado = normalizar(texto_original)

        registrar("REQUISICAO", user_id, texto_original)

        resposta_faq = buscar_faq(texto_normalizado)
        if resposta_faq:
            registrar("FAQ", user_id, "resposta encontrada no arquivo faq.py")
        ...
        if parece_pergunta_de_produto(texto_normalizado):
            ...
            if not produto and config.EXIGIR_PRODUTO_CADASTRADO:
                logging.warning(
                    "PRODUTO_FORA_CATALOGO | usuario=%s | texto=%s",
                    user_id,
                    texto_original,
                )
            ...
            registrar("PRODUTO", user_id, f"produto encontrado: {produto['nome']}")
            return {
        ...
        registrar("IA_GERAL", user_id, "pergunta enviada para a IA")
        return {
            "origem": "ia_geral",
        ...
        logging.error("ERRO_CHAT | %s", erro, exc_info=True)
        return {
            "origem": "erro",
            "resposta": "Não consegui processar sua mensagem agora. Tente novamente em alguns instantes.",
        }